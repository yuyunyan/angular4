...
@Component({
  selector: 'az-customer-rfqs',
  templateUrl: './customer-rfqs.component.html',
  styleUrls: ['./customer-rfqs.component.scss']
})
export class CustomerRFQsComponent extends BaseBomSearchComponentComponent {
  private customerRfqResults;
  ...
  createRow(rfq) {
    return {
      ...
      targetPrice: rfq.targetPrice? rfq.targetPrice.toLocaleString() :rfq.qty,
      bomQty:rfq.bomQty,
      bomPartNo:rfq.bomPartNo,
      bomPrice:rfq.bomPrice,
      pontential:rfq.pontential,
      bomMfg:rfq.bomMfg,
      bomIntPartNo:rfq.BomIntPartNumber,
      priceDelta:rfq.priceDelta
    }
  }

  getDataForGrid(searchId, partNumber,rowOffset, rowLimit, sortCol, DescSort, params){
    this.bomService.getBomResultCustomerRfqs(searchId, partNumber,rowOffset, rowLimit, sortCol, DescSort).takeUntil(this.ngUnsubscribe.asObservable()).subscribe(
      data => {
  ...
  setHeightOfGrid(recordsCount)
  {
    ...
    document.getElementById('bomResultCustomerRfqGrid').style.height = height+'px';
  }
