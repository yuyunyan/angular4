import { Component, ElementRef, ViewChild, SimpleChange,Input, EventEmitter} from '@angular/core';
...
@Component({
  selector: 'az-allocation-po-window',
  templateUrl: './allocation-po-window.component.html'
})
export class AllocationPOWindowComponent{
  @Input() availabilityGridPoParent;
    ...
    _self.orderFulfillmentService
      ...
      .subscribe(data => {
        ...
        if(dataJson.errorMessage && dataJson.errorMessage.length > 0){
          ...
          if(_self.availabilityGridPoParent){
            _self.availabilityGridPoParent.neededQty = _self.neededQty - _self.allocateQty;
          }
  ...
  onPoNumberClicked(){
    let url = `/pages/purchase-orders/purchase-order-details;purchaseOrderId=${this.poId};versionId=${this.poVersionId}`;
